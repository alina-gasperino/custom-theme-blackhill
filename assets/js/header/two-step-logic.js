/*
 Requires: jQuery, bm.cookies, bm.analytics
...
$(function twoStepLogic() {
  ...
  if (bm && bm.cookies && bm.analytics) {
    ...
    $("form.js-two-step").each(function initTwoStepForm() {
      ...
      $this_form.on("submit", function twoStepFormSubmitHandler(e) {
        ...
        var redirect_uri = $this_form.attr("action") || "/features/signup.jsp",
          cookie_val = "",
          bm_domain = /(.bm.net)/.test(document.location.host)
            ? ".bm.net"
            : ".constantcontact.com" // Unless we're on bm.net, enable all constantcontact.com URLs
    ...
    $("a.js-try-it-free, a.js-buy-now").each(function initBuyNowLinks() {
      ...
      $this.on("click", function buyNowLinkClickHandler(e) {
        ...
        $the_form
          .find("input")
          .each(function initTwoStepFormCookieFromBuyNowLink() {
            var $this_input = $(this)
            cookie_val +=
              $this_input.attr("name") + "=" + $this_input.val() + "|"
          })
  ...
  $("#signupform:not(.js-no-autofocus) input").each(
    function focusOnFirstErroredOrEmptyField() {
